...
\begin{abstract}
暗号で保護されたトランザクションと組み合わせたことで、ブロックチェーンは、多くのプロジェクトを通じてその有用性が実証されました.特に、Becoinは最も注目されています。 このような各プロジェクトは、分散化されたシングルトンのコンピューティングリソース上のシンプルなアプリケーションと見ることができます。 我々は、このパラダイムをstateを共有したトランザクション型シングルトンマシンと呼ぶことができます。

Ethereumはこのパラダイムを一般化された方法で実装します。 さらに、それは、それぞれ異なる状態および動作コードを有するが、メッセージの送受信フレームワークを介して他のコンピュターと相互作用することができる複数のリソースを提供します。 私たちは、その設計、実装の問題、それが提供する機会、そして我々が想定する将来のハードルについて議論します。
\end{abstract}
